window.Test_Cases = window.classes.Test_Cases =
class Test_Cases
{ static data()
  { return {
empty: `
...
TOP     1
RES     64 64
SPHERE  0 0 -10   2 4 2   0.5 0   0     0.5 1 0.9 50 .9 0 0.8
SPHERE  4 4 -10   1 2 1   0   0.5 0     0.5 1 0.9 50 .9 0 0.8
SPHERE -4 2 -10   1 2 1   0   0   0.5   0.5 1 0.9 50 .9 0 0.8
...
show_homework_spec: `
CS 174A (Graphics): Assignment 3     Weight: 5% Bonus, divided into 40 points
...
play with a working solution of this project online, use the blue button on it.  Your
...
Ball::constructor():  You are provided with a Ball class with some data members that
  automatically get filled in by Ray_Tracer::parse_file(). Ray_Tracer already stores
  an array of Balls - and passes in correct values to populate Ball's data members 
  when it does - so most of the work of constructor() is already done for you. Using 
  those for calculation, your job is to simply finish filling in any extra data 
  members that you think of.  You may think of more to add in later. Once this 
  function is finished, functionality to raster the image (turn ray tracing off) will
  work.
...
Ray_Tracer::trace():  Given a ray, return the color in that ray's path.  The ray
  ...
  refraction off of a ball.  Call Ball's intersect() method on each ball to 
...
Several samples of these scenes are provided in test-cases.js.
...
= If you create shadow rays as a vector directly from the closest hit point to the
  ...
  and 1 assuming the ray goes all the way to the light source. To deal with 
  floating point roundoff, you should not consider an intersection at time 0 to be 
  blocking the light from the object.
...
vec3 pixel_color = surface_color + (white - surface_color) * 
               ( k_r * trace(....).to3() + k_refract * trace(....).to3() )

- Be sure to clamp each component of surface_color, the above vec3, to at most 1.
- When multiplying two colors, do coefficient-wise products using Vec::mult_pairs().
- Since this is javascript, operators are not defined; forgetting to convert just 
  a single operator like * or - into a times, mult_pairs, plus, or minus could
  lead to an undefined crash.  This will happen a lot.
...
- There are three possibilities if you intersect a sphere: The near plane could
  ...
  Rastered graphics, shown with ray tracing off, don't apply this correction.
...
- Submit this on CCLE in a .zip archive, and name the file your bruin ID.  Include
  the whole template (all files, same folders and names, no extras) in your submission.
  `
      };
  }
}